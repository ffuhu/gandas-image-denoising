...
def image_write_tiff(path_A, path_B, path_AB):
    im_A = tifffile.imread(path_A)
    im_B = tifffile.imread(path_B)
    im_AB = np.concatenate([im_A, im_B], 1)
    tifffile.imwrite(path_AB, im_AB)
...
home_dir = '/home/ffuentes/'

# Convallaria_diaphragm
args.fold_A = home_dir + 'data/Convallaria_diaphragm/fupn2v_pix2pix_uint16/A'
...
for sp in splits:
    ...
    for n in range(num_imgs):
        ...
        if os.path.isfile(path_A) and os.path.isfile(path_B):
            ...
            if not args.no_multiprocessing:
                # pool.apply_async(image_write_tiff, args=(path_A, path_B, path_AB))
                pool.apply_async(image_write, args=(path_A, path_B, path_AB))
            else:
                # image_write_tiff(path_A, path_B, path_AB)
